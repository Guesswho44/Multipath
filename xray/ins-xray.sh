...
BURIQ () {
    curl -sS https://raw.githubusercontent.com/Guesswho44/Multipath/main/accesss > /root/tmp
...
Name=$(curl -sS https://raw.githubusercontent.com/Guesswho44/Multipath/main/accesss | grep $MYIP | awk '{print $2}')
...
PERMISSION () {
    MYIP=$(curl -sS ipv4.icanhazip.com)
    IZIN=$(curl -sS https://raw.githubusercontent.com/Guesswho44/Multipath/main/accesss | awk '{print $4}' | grep $MYIP)
    if [ "$MYIP" = "$IZIN" ]; then
    Bloman
    else
